...
async def get_nonce(request, vk):
    ...
    pending_nonce = metadata_driver.get_pending_nonce(processor=VK, sender=bytes.fromhex(vk))

    if pending_nonce is None:
        nonce = metadata_driver.get_nonce(processor=VK, sender=bytes.fromhex(vk))
        if nonce is None:
            pending_nonce = 0
        else:
            pending_nonce = nonce
